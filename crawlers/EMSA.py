...
baseurl = f"http://www.{AGENCY.lower()}.europa.eu"
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.field_text').text
...
def process_result_elem(result_elem):
    ...
    href = result_elem.select_one('h2 > span > a')["href"]

    url = make_url(href)
    # input(url)

    date = result_elem.select_one('div.field_created').text.strip().replace(" ", "")
    # input(date)
...
while go_on:
    ...
    r_p_url = f"https://www.emsa.europa.eu/newsroom/latest-news.html?start={i*10}?"
    ...
    results = r_p_soup.select(".fc_bloglist_item")
    # print(len(results))
    # input()
