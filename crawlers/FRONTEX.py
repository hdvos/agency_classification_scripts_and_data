...
def make_results_page_url(i:int) -> str:
    return f"https://frontex.europa.eu/media-centre/news/news-release/?p={i}"
...
def process_result_elem(result_elem):
    ...
    date = result_elem.select_one('span.date').text.strip().replace(" ", "")
    # print(date)
...
while go_on:
    ...
    r_p_url = f"https://frontex.europa.eu/media-centre/news/news-release/?p={i}"
    ...
    if r_p_response.status_code == 404:
        break #Website throws 404 error if there are no more results
