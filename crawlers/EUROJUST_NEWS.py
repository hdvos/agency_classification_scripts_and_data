...
def process_page(url, full_json, title, date, text_type):
    # input(url)
    ...
    try:
        text = soup.select_one('.field--text-content').text
...
def process_result_elem(result_elem):
    ...
    href = result_elem.select_one('h3 > a')["href"]

    url = make_url(href)
    # input(url)

    date = result_elem.select_one('.info-date').text.strip().replace(" ", "")
    # print(date)
    date = parse_date(date)

    text_type = result_elem.select_one('.info-extra').text
    # input(text_type)

    title = result_elem.select_one('h3').text.strip()
    # input(title)

    process_page(url, full_json, title, date, text_type)
# # #     # input()
...
while go_on:
    ...
    r_p_url = f"https://www.eurojust.europa.eu/media-and-events/press-releases-and-news?search=&year%5Bmin%5D=&year%5Bmax%5D=&joint_release=All&state=All&third_states=All&page={i}"
