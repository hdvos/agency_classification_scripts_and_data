...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.pageContents').text
...
def process_result_elem(result_elem):
        ...
        date = result_elem.select_one('div.description').text.strip()
        # # print(date)
        date = parse_date(date)
        # input(date)

        title = result_elem.select_one('a').text.strip()
        # input(title)
...
for year in range(2007, 2020+1):
    ...
    r_p_url = f"http://www.eurojust.europa.eu/press/PressReleases/Pages/{year}.aspx"
    # input(r_p_url)
    if i == 10 and DEBUG:
        break
    ...
    results = r_p_soup.select("li.dfwp-item")
