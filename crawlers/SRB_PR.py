...
def make_results_page_url(i:int) -> str:
    return f"https://srb.europa.eu/en/news/press-releases?page={i}"
...
def make_ID(text_type:str , agency_abbr:str) -> str:
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.post__body').text
...
def process_result_elem(result_elem):
        ...
        date = result_elem.select_one('div.simple-listing__date:nth-child(1) > span:nth-child(2)').text.strip()
        # print(date)
...
while go_on:
    ...
    # input()
    r_p_response = requests.get(r_p_url, verify = False)
