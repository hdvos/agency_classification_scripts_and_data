...
def make_results_page_url(i:int) -> str:
    return f"https://www.era.europa.eu/events-news/press-releases_en?page={i}"
...
def process_page(url, full_json, title, date, text_type):
    ...
    try:
        text = soup.select_one('.field-items').text
...
def process_result_elem(result_elem):
        ...
        # input(url)
        day = int(result_elem.select_one('.ecl-date-block__day').text.strip())
        month = result_elem.select_one('.ecl-date-block__month').text.strip()
        month = datedict[month]
        year = int(result_elem.select_one('.ecl-date-block__year').text.strip())

        date = datetime(year, month, day).strftime('%Y-%m-%d')
        # date = parse_date(date)
        # input(date)

        title = result_elem.select_one('.font-weight-bold').text
        # input(title)
