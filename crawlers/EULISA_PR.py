...
# The place where the output files will go
output_folder = f'/home/hugo/MEGA/work/Agency_Classification/crawling_files/{AGENCY}_DOCUMENTS/'
...
def make_results_page_url(i: int) -> str:
    return f"https://fra.europa.eu/en/news-and-events/news?page={i}"
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one(
            '#DeltaPlaceHolderMain > div:nth-child(1) > div.rich-text').text
...
def process_result_elem(result_elem):
    ...
    title = result_elem.select_one('h2').text.strip()
    # input(title)
...
with webdriver.Firefox() as driver:
    driver.get("https://www.eulisa.europa.eu/News-And-Events/Press-Releases")
    ...
    while go_on:
        ...
        results = r_p_soup.select(".infos-news")
        print(len(results))
        ...
        nextbtn = r_p_soup.select(".ms-promlink-button-enabled")
        if len(results) < 4:
            print("No nxtbtn")
            go_on = False
        nxt = driver.find_element_by_css_selector(
            '.next-page > a:nth-child(1)')
        nxt.click()
...
#     results = r_p_soup.select(".NewsItem")
#     # print(len(results))
#     # input()
