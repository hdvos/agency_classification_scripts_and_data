...
baseurl = "https://www.bbi-europe.eu"
# baseurl = f"https://www.{AGENCY.lower()}.europa.eu"
# baseurl = f"https://{AGENCY.lower()}.europa.eu"
...
# csv_file = f"{AGENCY}_PR.csv"
# json_file = f"{AGENCY}_PR.json"
...
def make_results_page_url(i:int) -> str:
    return f"https://fra.europa.eu/en/media/press-releases?page={i}"
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.field-name-body').text
...
def process_result_elem(result_elem):
        ...
        href = result_elem.select_one('a')["href"]

        url = make_url(href)
        

        date = result_elem.select_one('.date-display-single').text.strip()
        # # print(date)
        date = parse_date(date)
        # input(date)

        title = result_elem.select_one('span.field-content').text.strip()
        # input(title)
...
"https://www.bbi-europe.eu/news"

result_page_resp = requests.get("https://www.bbi-europe.eu/news")
result_page_html = result_page_resp.text
result_page_soup = BeautifulSoup(result_page_html, features='lxml')

results = result_page_soup.select("div.view-content > .views-row")
print(len(results))

results.pop(0) # remove the first item of results. since it is someho empty
