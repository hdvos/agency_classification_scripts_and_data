...
def make_results_page_url(i:int) -> str:
    return f"https://www.cepol.europa.eu/media/news?page={i}"
...
def process_result_elem(result_elem):
        ...
        title = result_elem.select_one('.views-field-title').text.strip()
        # input(title)
...
while go_on:
    ...
    results = r_p_soup.select("#block-system-main > .block-inner > .content > .view > .view-content > .views-row")
