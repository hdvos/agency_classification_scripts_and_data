...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.node-body').text
...
def process_result_elem(result_elem):
        ...
        href = result_elem.select_one('a.link-zone')["href"]
        ...
        title = result_elem.select_one('.news-title').text.strip()
        # input(title)
...
for year in range(2017, 2020+1):
    for month, ndays in zip(range(1,12+1), [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]):
        r_p_url = f"https://www.cdt.europa.eu/en/news/archives/{year}{month:02d}01--{year}{month:02d}{ndays}"
        
        print(f"Trying {year} - {month:02d}")
        # if i == 10 and DEBUG:
        #     break
        ...
        if r_p_response.status_code == 404:
            print("\t404")
        # try:
        ...
        results = r_p_soup.select(".listing-news")
