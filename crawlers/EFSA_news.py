...
def make_results_page_url(i:int) -> str:
    return f"https://www.efsa.europa.eu/en/news?page={i}"
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.news__field-paragraphs').text
...
def process_result_elem(result_elem):
        ...
        try:
            date = result_elem.select_one('time').text.strip()
            print(date)
        ...
        title = result_elem.select_one('h3.card-title').text.strip()
        # input(title)
...
while go_on:
    ...
    results = r_p_soup.select(".card-default-newsroom")
    print(len(results))
    try:
        results.pop(0)
    except IndexError:
