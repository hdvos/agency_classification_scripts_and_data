...
def make_results_page_url(i:int) -> str:
    return f"https://www.europol.europa.eu/newsroom?page={i}"
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.body.field').text
...
def process_result_elem(result_elem):
        ...
        href = result_elem.select_one('a.group-link-wrapper')["href"]

        url = make_url(href)
        # input(url)

        date = result_elem.select_one('.date-elements').text.strip()
        # print(date)
        # input()
...
while go_on:
    ...
    results = r_p_soup.select(".group-content")
    # print(len(results))
    # input()
