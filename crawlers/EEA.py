...
baseurl = "https://www.eea.europa.eu/"
...
def make_results_page_url(i:int) -> str:
    number = i*10
    return f"https://www.eea.europa.eu/highlights/archive/@@faceted_query?c7%5B%5D=en&c6%5B%5D=&b_start:int={number}&version=08269d5c87d5dcb12857f294aedf232e"
...
def process_page(url,date_elem_list, full_json):
    ...
    try:
        title = soup.select("#parent-fieldname-title")[0].text.strip('\n').strip()
    except:
        title = "NULL"

    print(f'\t- {title}')
    try:
        date = date_elem_list[0].text.strip()
    ...
    try:
        text = soup.select('#content-core')[0].text.strip()
...
while go_on:
    ...
    results = r_p_soup.select("div.tileItem > div:nth-child(2)")
    ...
    for result_elem in results:
        ...
        date_elem_list = result_elem.select('.date')

        url = make_url(href_elem_list)

        process_page(url, date_elem_list, full_json)
