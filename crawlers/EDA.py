...
def make_results_page_url(i:int) -> str:
    return f"https://www.eda.europa.eu/info-hub/press-centre/latest-news/page/{i}"
...
def make_url(href:list) -> str:
    baseurl = "https://www.eda.europa.eu/info-hub/press-centre/latest-news/"
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.sfnewsContent').text
...
def process_result_elem(result_elem):
        ...
        date = result_elem.select_one('.eda_nl_date').text.strip()
        date = date.split("-")[-1].strip()
        # print(date)
        date = parse_date(date)
        # input(date)

        title = result_elem.select_one('h3.eda_nl_title').text.strip()
        # input(title)
...
while go_on:
    ...
    results = r_p_soup.select(".eda_nl_item")
    # print(len(results))
    # input()
