...
def make_results_page_url(i:int) -> str:
    return f"https://www.enisa.europa.eu/news?page:int={i}&ptype:list=News%20Item&ptype:list=Press%20Release"
...
def process_page(url, full_json, title, date, text_type):
    ...
    text = soup.select_one('article > div').text
...
while go_on:
    ...
    results = r_p_soup.select(".news-latest-item")
    # print(len(results))
    ...
    for result_elem in results:
        ...
        # input(url)
        date = result_elem.select_one('.news-latest-date').text.strip()
        date = parse_date(date)

        title = result_elem.select_one('.news-latest-title').text
        # input(title)

        text_type = result_elem.select_one(".news-latest-type").text.strip()
