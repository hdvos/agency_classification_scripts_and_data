...
def make_results_page_url(i:int) -> str:
    return f"https://easo.europa.eu/news-events/press-releases?page={i}"
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.news-content').text
...
while go_on:
    ...
    results = r_p_soup.select("div.view:nth-child(2) > div:nth-child(1) > div")
    print(len(results))
