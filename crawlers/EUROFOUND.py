...
def make_results_page_url(i:int) -> str:
    return f"https://www.eurofound.europa.eu/news/news-articles?page={i}"
...
def process_result_elem(result_elem):
        ...
        date = result_elem.select_one('.views-field-published-at').text.strip()
        # print(date)
...
while go_on:
    ...
    results = r_p_soup.select("div.main > div > div.view > div.view-content > div.item-list > ul > li.views-row")
    # print(len(results))
    # input()
