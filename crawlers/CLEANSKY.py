...
def make_results_page_url(i:int) -> str:
    return f"https://www.cleansky.eu/news?page={i}"
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('div.text-formatted:nth-child(4)').text
...
def process_result_elem(result_elem):
        ...
        # print(result_elem)
        # href = result_elem.select_one('.cs_customLink')["href"]
        
        href = result_elem.select_one('.view-mode-teaser')["onclick"]
        ...
        date = result_elem.select_one('.field--name-field-date').text.strip()
        # # print(date)
