...
baseurl = f"https://www.{AGENCY.lower()}.eu"
...
def make_results_page_url(i:int) -> str:
    return f"https://www.ecsel.eu/news"
...
def process_page(url, full_json, title, date):
    ...
    try:
        text = soup.select_one('.stack-body').text
...
def process_result_elem(result_elem):
        ...
        href = result_elem.select_one('a.overlink')["href"]

        url = make_url(href)
        # input(url)

        date = result_elem.select_one('div.date').text.strip()
        date = date.replace("Posted on", "").strip()
        # print(date)
        date = parse_date(date)
        # input(date)

        title = result_elem.select_one('h2.title').text.strip()
        # input(title)
